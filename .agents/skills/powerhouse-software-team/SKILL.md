...
| uild | Invoke Dev Engineer -> implement current ticket |
| eview | Invoke QA Engineer -> validate changes |
| ranch [name] | Create feature/{subproject}/L{N}-{name} |
| pr | Type-check -> rebase -> push -> create PR |
| 
ext | Find first unchecked item across all sub-projects |
...
- Branch naming: eature/{subproject}/L{N}-{name}
